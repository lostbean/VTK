...
renderType dataSet = case dataSet of
    ...
    RectLinGrid x y z ->
        let
         ...
         in
            ("RectilinearGrid", renderWholeExtAttr ext)
...
renderSP ::
    ...
    [VTKAttrPoint a] ->
    Xml Elem
renderSP isBin ext size pointData =
    let
        nodes = [renderPointData isBin size pointData]
     in
        xelem "Piece" $ xattr "Extent" (renderExtent ext) <#> xelems nodes
...
renderRG ::
    ...
    [VTKAttrPoint a] ->
    Xml Elem
renderRG isBin setX setY setZ size pointData =
    let
        ext = (U.length setX, U.length setY, U.length setZ)
        nodes =
            [ renderPointData isBin size pointData
...
renderUG ::
    ...
    [VTKAttrCell a] ->
    Xml Elem
renderUG isBin set cell cellOff cellType pointData cellData =
    let
        numPoints = U.length set
        numCell = U.length cellOff
        attrs =
            [ xattr "NumberOfPoints" (toTxt numPoints)
            , xattr "NumberOfCells" (toTxt numCell)
            ]
        nodes =
            [ renderPointValueData isBin set pointData
     ...
     in
        xelem "Piece" (xattrs attrs <#> xelems nodes)

-- =============================== render tools ==========================================

renderPointData :: (RenderElemVTK a) => Bool -> Int -> [VTKAttrPoint a] -> Xml Elem
renderPointData isBin size attrs =
    let
        renderAttr (VTKAttrPoint name func) =
            let
                points = U.generate size func
                name_t = toTxt name
             in
                renderDataArray isBin points name_t
     in
        xelem "PointData" $ xelems $ P.map renderAttr attrs

renderPointValueData :: (RenderElemVTK a) => Bool -> Vector a -> [VTKAttrPointValue a] -> Xml Elem
renderPointValueData isBin pointData attrs =
    let
        renderAttr (VTKAttrPointValue name func) =
            let
...
renderCellData ::
    ...
    [VTKAttrCell a] ->
    Xml Elem
renderCellData isBin set cell cellOff cellType attrs =
    let
        renderAttr (VTKAttrCell name func) =
            let
                arr = U.imap (mode func) cellOff
             in
                renderDataArray isBin arr (toTxt name)
        mode func i _ =
            let
                sec =
                    U.map (set !) $
             ...
             in
                func i sec tp
     in
        xelem "CellData" $ xelems $ P.map renderAttr attrs
...
renderCells ::
    ...
    Vector Int ->
    Xml Elem
renderCells isBin cellConn cellOffsets cellTypes faces faceOffsets =
    let
...
renderExtent (dx, dy, dz) =
    let
        zr = toTxt (0 :: Int)
     in
        T.unwords [zr, toTxt (dx - 1), zr, toTxt (dy - 1), zr, toTxt (dz - 1)]
...
renderDataArray isBin points name =
    let
        ...
        format
            ...
            | otherwise = "ascii"
        attr =
            [ xattr "type" (getNumType points)
            , xattr "Name" name
            , xattr "format" format
            , xattr "NumberOfComponents" (getNumComp points)
            ]
        runrender = xtextRaw . BB.lazyByteString . vecRender
     in
        xelem "DataArray" ((xattrs attr) <#> runrender points)
