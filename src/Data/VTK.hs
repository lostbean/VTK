...
-}
mkUGVTK ::
    (RenderElemVTK p, RenderCell shape, FastFoldable cont shape) =>
    String -> Vector p -> cont shape -> [VTKAttrPointValue p] -> [VTKAttrCell p] -> VTK p
mkUGVTK name points cells pointData cellData =
    let
        nameTxt = pack name
        dataset = mkUnstructGrid points cells
     in
        VTK nameTxt dataset pointData [] cellData
...
mkRLGVTK ::
    ...
    [VTKAttrPoint a] ->
    VTK a
mkRLGVTK name px py pz pointData =
    let
        nameTxt = pack name
        dataset = mkRectLinGrid px py pz
     in
        VTK nameTxt dataset [] pointData []
...
mkSGVTK name nx ny nz pointData =
    let
        nameTxt = pack name
...
mkSPVTK ::
    ...
    [VTKAttrPoint a] ->
    VTK a
mkSPVTK name (dx, dy, dz) orig spc pointData =
    let
        nameTxt = pack name
        dataset =
            StructPoint
